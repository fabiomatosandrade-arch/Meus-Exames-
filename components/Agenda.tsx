...
const Agenda: React.FC<AgendaProps> = ({ appointments, setAppointments, doctors, laboratories }) => {
  ...
  const [isGoogleConnected, setIsGoogleConnected] = useState(() => {
    return localStorage.getItem('lifeTrace_google_connected') === 'true';
  });
  ...
  const toggleGoogleConnection = () => {
    ...
    localStorage.setItem('lifeTrace_google_connected', String(newState));
    if (newState) {
      alert("Conectado ao Google Agenda! Novos compromissos abrirão o Google Calendar automaticamente.");
    }
  };
  ...
  const getGoogleCalendarLink = (app: Appointment) => {
  ...
  const syncAllWithGoogle = () => {
    const future = sortedAppointments.filter(app => new Date(`${app.date}T${app.time}`) >= new Date());
    if (future.length === 0) {
      alert("Não há compromissos futuros para sincronizar.");
      return;
    }
    if (confirm(`Deseja exportar ${future.length} compromisso(s) para o Google Agenda?`)) {
      future.forEach((app, idx) => {
        setTimeout(() => {
          window.open(getGoogleCalendarLink(app), '_blank');
        }, idx * 800); 
      });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-800 tracking-tight">Agenda de Saúde</h2>
          <p className="text-slate-500">Gestão de consultas e exames com sincronização</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <button 
            onClick={toggleGoogleConnection}
            className={`p-3 rounded-xl border transition-all flex items-center gap-2 text-[10px] font-black uppercase shadow-sm ${isGoogleConnected ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-white text-slate-400 border-slate-200 hover:border-blue-200'}`}
          >
            <Link2 className="w-4 h-4" />
            {isGoogleConnected ? 'Google Conectado' : 'Logar com Google'}
          </button>
          <button 
            onClick={syncAllWithGoogle}
            className="p-3 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-blue-50 transition-all flex items-center gap-2 text-[10px] font-black uppercase shadow-sm"
          >
          ...
          >
            ...
            {isSoundEnabled ? "Alertas Ativos" : "Alertas Mudos"}
          </button>
          <button
        ...
        <div className="lg:col-span-2 space-y-4">
          {sortedAppointments.length === 0 ? (
            <div className="py-20 text-center bg-white rounded-[40px] border border-dashed border-slate-300">
              ...
              <p className="text-slate-500 font-black uppercase tracking-widest text-[10px]">Sem compromissos marcados.</p>
            </div>
          ) : (
            sortedAppointments.map(app => {
              const happeningNow = isHappeningNow(app.date, app.time);
              return (
                <div key={app.id} className={`bg-white p-6 rounded-[32px] border transition-all flex flex-col sm:flex-row items-center justify-between gap-6 hover:shadow-md ${happeningNow ? 'border-amber-400 bg-amber-50 ring-4 ring-amber-100 animate-pulse' : 'border-slate-100 shadow-sm'}`}>
                   <div className="flex items-center gap-5 w-full sm:w-auto">
                      ...
                      <div className="flex-1">
                         <h3 className="text-lg font-black text-slate-800 uppercase leading-tight tracking-tight">{app.title}</h3>
                         <div className="flex items-center gap-3 mt-1.5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                            <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-blue-400" /> {app.time}</span>
                            <span className="flex items-center gap-1 truncate max-w-[150px]"><Landmark className="w-3.5 h-3.5 text-indigo-400" /> {app.location}</span>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center gap-2 w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0 justify-end">
                      <button 
                        ...
                        className="bg-white border border-slate-200 text-slate-600 p-3 rounded-xl hover:bg-blue-50 transition-all flex items-center gap-2 text-[9px] font-black uppercase tracking-widest shadow-sm"
                          ...
                          {activeNavMenu === app.id && (
                            <div className="absolute bottom-full right-0 mb-2 w-40 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden">
                               <button onClick={() => openNavigation(app.address!, 'google')} className="w-full p-3 text-left text-[10px] font-black text-slate-700 hover:bg-blue-50 flex items-center gap-2">
                                 <MapIcon className="w-4 h-4 text-emerald-500" /> Google Maps
                               </button>
        ...
        <div className="space-y-6">
           <div className="bg-slate-900 p-8 rounded-[40px] text-white shadow-xl relative overflow-hidden">
              ...
              <h3 className="text-xl font-black mb-4 uppercase tracking-tight flex items-center gap-2">
                <Bell className="w-6 h-6 text-blue-400" /> Alerta de Saúde
              </h3>
              <p className="text-slate-400 text-[10px] leading-relaxed mb-6 font-black uppercase tracking-widest">
                Mantenha a página aberta para receber alertas sonoros. Sincronize com o Google para notificações no celular.
              </p>
              <button 
           ...
           <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-4">
              <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                <CalendarIcon className="w-4 h-4 text-blue-500" /> Próximos 30 dias
              </h4>
              <div className="flex items-end gap-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase pb-1 mb-1">Eventos</span>
              </div>
           </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-blue-600 text-white">
              <h3 className="text-xl font-black uppercase tracking-tight">Agendar Compromisso</h3>
            ...
            <form onSubmit={handleAdd} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto">
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Título do Compromisso</label>
                <input 
                  type="text" required 
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-bold uppercase text-sm" 
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Local / Estabelecimento</label>
                <input 
                  type="text" 
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-bold uppercase text-sm" 
              ...
              <div>
                ...
                <input 
                  type="text" 
                  className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-bold uppercase text-xs" 
              ...
              <button type="submit" className="w-full bg-blue-600 text-white font-black py-5 rounded-2xl shadow-lg hover:bg-blue-700 transition-all uppercase tracking-widest flex items-center justify-center gap-3">

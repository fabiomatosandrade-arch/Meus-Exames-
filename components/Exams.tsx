...
import { 
  ...
  BarChart, Edit3, List, User as UserIcon, FileSearch, ExternalLink
} from 'lucide-react';
...
const Exams: React.FC<ExamsProps> = ({ exams, setExams, doctors, setDoctors, laboratories, setLaboratories }) => {
  ...
  const [currentFileUri, setCurrentFileUri] = useState<string | undefined>(undefined);
  const [currentFileMime, setCurrentFileMime] = useState<string | undefined>(undefined);
  ...
  const viewAttachment = (exam: ExamRecord) => {
  ...
  const handleSaveExtracted = () => {
    ...
    setExtractedExams([]);
    setCurrentFileUri(undefined);
    setCurrentFileMime(undefined);
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-5xl rounded-[48px] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in duration-300">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-blue-600 text-white">
              ...
              <button onClick={() => { setShowAddModal(false); setExtractedExams([]); setCurrentFileUri(undefined); }} className="p-2 hover:bg-white/10 rounded-full transition-colors"><X className="w-8 h-8" /></button>
            </div>
              ...
              {extractedExams.length > 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                   <div className="space-y-8">
                    <div className="bg-white rounded-[40px] p-10 border-4 border-dashed border-blue-100 text-center hover:bg-blue-50/50 hover:border-blue-300 transition-all cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
                      {isAnalyzingFile ? (
                        ...
                        <div className="py-16 flex flex-col items-center gap-8">
                           <div className={`p-8 rounded-[32px] shadow-2xl transition-transform ${currentFileUri ? 'bg-emerald-500' : 'bg-blue-600'} text-white`}>
              ...
              <div className="p-12 overflow-y-auto">
                 <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12">
                   <div className="flex-1 w-full">
                      ...
                      <div className="flex items-center gap-3">
                         <span className={`text-[10px] font-black uppercase tracking-widest px-5 py-2 rounded-full border ${statusTextColors[getHealthStatus(viewingExamRecord.value, viewingExamRecord.referenceRange)]} ${statusColors[getHealthStatus(viewingExamRecord.value, viewingExamRecord.referenceRange)]} bg-opacity-10`}>{statusLabels[getHealthStatus(viewingExamRecord.value, viewingExamRecord.referenceRange)]}</span>
                 ...
                 <div className="pt-10 border-t border-slate-100 flex justify-between items-center">
                    <div className="flex gap-4">
                       ...
                       {viewingExamRecord.fileUri && (
                         <button onClick={() => viewAttachment(viewingExamRecord)} className="flex items-center gap-2 text-emerald-600 font-black text-[10px] uppercase tracking-widest hover:underline"><FileSearch className="w-4 h-4" /> Ver Arquivo Original</button>
                       )}
                    </div>

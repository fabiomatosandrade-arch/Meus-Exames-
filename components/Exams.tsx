...
export const getHealthStatus = (valueStr: string, referenceRange: string) => {
  ...
  if (isNumericValue) {
    ...
    if (greaterMatch) {
      ...
      if (val <= min && val >= min * 0.9) return 'warning';
      return 'danger';
    }
    ...
    if (lessMatch) {
      ...
      if (val < max) return 'success';
      if (val >= max && val <= max * 1.1) return 'warning';
      return 'danger';
    }
  } else {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        if (extracted && extracted.length > 0) {
          ...
          if (normalizedExtracted.length === 1) {
            setNewExam(prev => ({ ...prev, ...normalizedExtracted[0] }));
            if (normalizedExtracted[0].examName) {
              const info = await getExamInformation(normalizedExtracted[0].examName);
              setInfoBox(info);
            }
          } else {
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 print:hidden">
         ...
         >
            ...
            <div className="relative z-10">
               <span className="text-[11px] font-black uppercase tracking-widest block mb-1">Total Geral</span>
               <div className="w-full bg-slate-200/20 h-1 rounded-full overflow-hidden">
                  <div className="bg-blue-400 h-full rounded-full w-full"></div>
               </div>
            </div>
         </button>
         ...
         >
            ...
            <div className="relative z-10">
               ...
               <div className="w-full bg-slate-200 h-1 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ${statusFilter === 'success' ? 'bg-emerald-300' : 'bg-emerald-500'}`} 
                    style={{ width: `${(statsCount.success / (exams.length || 1)) * 100}%` }}
                  ></div>
               </div>
            </div>
         </button>
         ...
         <button 
           ...
           className={`p-5 rounded-[32px] border transition-all text-left flex flex-col justify-between h-32 relative overflow-hidden group ${statusFilter === 'warning' ? 'bg-amber-500 border-amber-600 text-white shadow-xl ring-4 ring-amber-100 ring-offset-2' : 'bg-white border-slate-100 text-slate-600 hover:border-amber-200 shadow-sm'}`}
         >
            <div className="flex items-center justify-between relative z-10">
            ...
            <div className="relative z-10">
               ...
               <div className="w-full bg-slate-200 h-1 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ${statusFilter === 'warning' ? 'bg-amber-200' : 'bg-amber-500'}`} 
                    style={{ width: `${(statsCount.warning / (exams.length || 1)) * 100}%` }}
                  ></div>
               </div>
            </div>
         </button>
         ...
         >
            ...
            <div className="relative z-10">
               ...
               <div className="w-full bg-slate-200 h-1 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ${statusFilter === 'danger' ? 'bg-rose-300' : 'bg-rose-500'}`} 
                    style={{ width: `${(statsCount.danger / (exams.length || 1)) * 100}%` }}
                  ></div>
               </div>
            </div>
         </button>
         ...
         >
            ...
            <div className="relative z-10">
               <span className="text-[11px] font-black uppercase tracking-widest block mb-1">Indefinidos</span>
               <div className="w-full bg-slate-200 h-1 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ${statusFilter === 'neutral' ? 'bg-slate-200' : 'bg-slate-400'}`} 
                    style={{ width: `${(statsCount.neutral / (exams.length || 1)) * 100}%` }}
                  ></div>
               </div>
            </div>
         </button>
